...
RUN --mount=type=cache,target=/root/.cache/pip \
    ...
    do \
        # Explicitly pin tracking markers before clone to isolate directory context
        START_DIR=$(pwd); \
        \
        ...
        \
        # Read the exact directory name that was generated dynamically by Git
        cd tmp_clone_dir && repo_dir=$(basename "$(pwd)") && cd ..; \
        mv tmp_clone_dir "$repo_dir"; \
        \
        echo "CIRCLECI_HEARTBEAT: Successfully resolved [$repo_dir]"; \
        \
        # Harmonize and Install Requirements
        ...
        \
        # Force return to parent root to prevent subfolder layout drift
        cd "$START_DIR"; \
    done
