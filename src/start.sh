...
# Check if SageAttention is installed in the Python environment
echo "🔍 Checking for SageAttention..."
if $PYTHON_BIN -c "import sageattention" &> /dev/null; then
    SAGE_ATTENTION_AVAILABLE=true
    echo "✅ SageAttention is available. ComfyUI will launch with acceleration."
else
    SAGE_ATTENTION_AVAILABLE=false
    echo "⚠️  SageAttention not found. Falling back to standard attention."
fi
...
status_msg "▶️ Executing: $COMFYUI_CMD"
